...
fn handle_key_press(key: Result<Key, Error>,
    ...
    match key.unwrap() {
        Key::Char('\n') => {
            let current_line = lines[(cursor.y as usize + terminal_height_offset) - 1].clone();
            let nb_char_in_current_line = current_line.len() as u16;
            let end_of_line =
                &current_line[cursor.x as usize - 1..nb_char_in_current_line as usize];
            lines.insert(cursor.y as usize+ terminal_height_offset, String::from(end_of_line));
            let current_line = &mut lines[(cursor.y as usize+ terminal_height_offset) - 1];
