...
fn backspace_remove_characters(y_position_in_file: usize, cursor: &mut CursorPosition, lines: &mut Vec<String>) {
    let current_line = &mut lines[y_position_in_file - 1];
    cursor.x = cursor.x - 1;
    current_line.remove(cursor.x as usize - 1);
}

fn backspace_remove_at_the_beginning_of_line_when_in_terminal(y_position_in_file: usize, cursor: &mut CursorPosition, lines: &mut Vec<String>) {
...
fn handle_key_press(key: Result<Key, Error>,
    ...
    match key.unwrap() {
        ...
        Key::Backspace => {
            if cursor.x != 1 {
                backspace_remove_characters(&y_position_in_file, &cursor, &lines);
                ...
                cursor.x = nb_char_in_previous_line + 1;
                lines.remove(y_position_in_file);
            } else if y_position_in_file > 1 && cursor.y == 1 {
                let nb_char_in_previous_line = lines[y_position_in_file - 2].len() as u16;
                cursor.x = nb_char_in_previous_line + 1;
                lines.remove(y_position_in_file);
            } else if cursor.y > 1 {
